...
## In Progress
- **[congress-trading-shared][CURSOR] Comprehensive repo audit + agent work incorporation — 2026-07-22.** Branch `cursor/land-antigravity-20260722` (based on `origin/main` `76fb945`). Merged Antigravity's `normalizeCompanyName()` utility with state-suffix stripping (197 lines, 6 tests, 853 total). grok3: net-zero (alias on main). codex v2: rejected (would delete compat alias). Consumers: ST→v1.11.1 (OUTDATED), CT→v2.0.0 (current), UM→v2.0.0 (current). Stale Claude worktree cleaned.
