...
## In Progress
- **[congress-trading-shared][CURSOR] Comprehensive repo audit + agent work incorporation — 2026-07-22.** PR #222 (`cursor/land-antigravity-20260722`). Merged Antigravity's `normalizeCompanyName()` + state-suffix stripping (197 lines, 6 tests, 853 total). grok3: skip (alias on main). codex v2: reject (would delete compat alias). Consumers: ST→v1.11.1 (OUTDATED), CT→v2.0.0 (current), UM→v2.0.0 (current). Stale Claude worktree removed.
